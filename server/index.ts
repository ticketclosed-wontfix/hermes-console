...
// Read bearer token from API_SERVER_KEY (matches .env). Legacy HERMES_API_KEY accepted.
...
// Local /api routes have no bearer check — they're local-only and the browser
// (same-origin) never sends Authorization. Auth is enforced only on the
// gateway proxy. See server/production.ts for detailed rationale.
